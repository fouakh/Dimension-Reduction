...
class HollowRectangle(SyntheticShape):
    def __init__(
        self,
        nx: int,
        ny: int,
        ...
        if 2 * inner_margin_x >= nx or 2 * inner_margin_y >= ny:
            raise ValueError("Inner margins too large: hollow removes everything.")
    ...
    def samples(self) -> np.ndarray:
        ...
        mask_x = (
            (X[:, 0] < self.inner_margin_x) |
            (X[:, 0] >= self.nx - self.inner_margin_x)
        )
        mask_y = (
